...
void PageMarkBitMap::print_on_error(outputStream* st, const char* prefix) const {
  _bm.print_on_error(st, prefix);
}

size_t PageMarkBitMap::compute_size(size_t heap_size) {
  return ReservedSpace::allocation_align_size_up(heap_size / mark_distance());
}

// For 4KB page
size_t PageMarkBitMap::mark_distance() {
  return 4096 * BitsPerByte;
}

void PageMarkBitMap::initialize(MemRegion heap, MemRegion storage) {
  _covered = heap;

  _bm = BitMapView((BitMap::bm_word_t*) storage.start(), _covered.word_size() >> _shifter);
}

void PageMarkBitMap::do_clear(MemRegion mr, bool large) {
...
#ifdef ASSERT
void PageMarkBitMap::check_mark(HeapWord* addr) {
