...
#ifndef SHARE_GC_SHARED_PAGEMARKBITMAP_INLINE_HPP
#define SHARE_GC_SHARED_PAGEMARKBITMAP_INLINE_HPP
...
inline HeapWord* PageMarkBitMap::get_next_marked_addr(const HeapWord* const addr,
...
inline void PageMarkBitMap::mark(HeapWord* addr) {
  check_mark(addr);
  _bm.set_bit(addr_to_offset(addr));
}

inline void PageMarkBitMap::mark(oop obj) {
  return mark(cast_from_oop<HeapWord*>(obj));
}

inline void PageMarkBitMap::clear(HeapWord* addr) {
  check_mark(addr);
  _bm.clear_bit(addr_to_offset(addr));
}

inline bool PageMarkBitMap::par_mark(HeapWord* addr) {
  check_mark(addr);
  return _bm.par_set_bit(addr_to_offset(addr));
}

inline bool PageMarkBitMap::par_mark(oop obj) {
  return par_mark(cast_from_oop<HeapWord*>(obj));
}

inline bool PageMarkBitMap::is_marked(oop obj) const{
  return is_marked(cast_from_oop<HeapWord*>(obj));
}

inline void PageMarkBitMap::clear(oop obj) {
  clear(cast_from_oop<HeapWord*>(obj));
}

#endif // SHARE_GC_SHARED_PAGEMARKBITMAP_INLINE_HPP
