...
class VM_ShenandoahUpdateRefs: public VM_ShenandoahOperation {
private:
    ShenandoahConcurrentGC* const _gc;
public:
  VM_ShenandoahUpdateRefs(ShenandoahConcurrentGC* gc) :
    VM_ShenandoahOperation(),
    _gc(gc) {};
  VM_Operation::VMOp_Type type() const { return VMOp_ShenandoahEvacuation; }
  const char* name()             const { return "Shenandoah Update Refs"; }
  virtual void doit();
  virtual bool allow_nested_vm_operations() const { return true; }
};
